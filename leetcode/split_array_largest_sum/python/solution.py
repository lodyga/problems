class Solution:
    def splitArray(self, numbers: list[int], k: int) -> int:
        """
        ...
        def dfs(index, k):
            ...
            memo[(index, k)] = total
            current_sum = 0
            for index2 in range(index, len(numbers) - k + 1):
                current_sum += numbers[index2]
                next_sum = dfs(index2 + 1, k - 1)
                larger_sum = max(current_sum, next_sum)
                memo[(index, k)] = min(memo[(index, k)], larger_sum)

                if current_sum > memo[(index, k)]:
                    break
    ...
    def splitArray(self, numbers: list[int], k: int) -> int:
        ...
        def dfs(index, k):
            ...
            for index2 in range(index, len(numbers) - k + 1):
                ...
                next_sum = dfs(index2 + 1, k - 1)
                larger_sum = max(current_sum, next_sum)
                min_sum = min(min_sum, larger_sum)
...
print(Solution().splitArray([5, 5], 2), 5)
