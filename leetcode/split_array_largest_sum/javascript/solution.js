class Solution {
   ...
   splitArray(numbers, k) {
      ...
      const dfs = (index, k) => {
         if (k === 1) {
            return numbers.slice(index,).reduce((sum, number) => sum + number, 0);
         } else if (memo.has(`${index},${k}`)) {
            return memo.get(`${index},${k}`)
         }

         let currentSum = 0;
         let minSum = total;
         for (let index2 = index; index2 < numbers.length - k + 1; index2++) {
            currentSum += numbers[index2];
            const nextSum = dfs(index2 + 1, k - 1);
            const largerSum = Math.max(currentSum, nextSum);
            minSum = Math.min(minSum, largerSum);

            if (currentSum > minSum) {
               break
            }
         }
         memo.set(`${index},${k}`, minSum)
         return minSum
      }
