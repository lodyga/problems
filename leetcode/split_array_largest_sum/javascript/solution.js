class Solution {
    ...
    * Auxiliary space complexity: O(k*n)
    * Tags:
    *    DS: hash map
    *    A: top-down
    * @param {number[]} nums
   ...
   splitArray(nums, k) {
      ...
      const dfs = (start, k) => {
         if (k === 1) {
            return nums.slice(start,).reduce((sum, num) => sum + num, 0);
         } else if (memo.has(`${start},${k}`)) {
            return memo.get(`${start},${k}`)
         }

         let res = total;
         let currentSum = 0;

         for (let index = start; index < nums.length - k + 1; index++) {
            currentSum += nums[index];
            const nextSum = dfs(index + 1, k - 1);
            const largerSum = Math.max(currentSum, nextSum);
            res = Math.min(res, largerSum);

            if (currentSum > res) {
               break
            }
         }
         memo.set(`${start},${k}`, res)
         return res
      }
