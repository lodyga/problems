class Solution {
   ...
   minCostClimbingStairs(costs) {
      const memo = Array(costs.length).fill(-1);
      memo.push(0, 0);
      ...
      const dfs = (idx) => {
         ...
         memo[idx] = cost + Math.min(dfs(idx + 1), dfs(idx + 2));
         return memo[idx]
      }
...
class Solution {
   ...
   minCostClimbingStairs(costs) {
      ...
      for (let idx = costs.length - 1; idx > -1; idx--) {
         const cost = costs[idx];
         cache[idx] = cost + Math.min(cache[idx + 1], cache[idx + 2]);
      }
...
class Solution {
   ...
   minCostClimbingStairs(costs) {
      ...
      for (let idx = costs.length - 1; idx > -1; idx--) {
         const cost = costs[idx];
         cache[idx % 2] = cost + Math.min(cache[0], cache[1]);
      }
