class Solution {
   ...
   minCostClimbingStairs(costs) {
      const memo = Array(costs.length + 2).fill(-1);
      memo[costs.length] = 0;
      memo[costs.length + 1] = 0;
      ...
      const dfs = (index) => {
         ...
         const one_step = cost + dfs(index + 1)
         const two_steps = cost + dfs(index + 2)

         memo[index] = Math.min(one_step, two_steps)
         return memo[index]
      }
    ...
    */
   minCostClimbingStairs(costs) {
      const cache = Array(costs.length + 2).fill(0);

      for (let index = costs.length - 1; index > -1; index--) {
         const cost = costs[index];
         cache[index] = cost + Math.min(cache[index + 1], cache[index + 2]);
      }
    ...
    */
   minCostClimbingStairs(costs) {
      const cache = Array(2).fill(0);

      for (let index = costs.length - 1; index > -1; index--) {
         const cost = costs[index];
         cache[index % 2] = cost + Math.min(cache[0], cache[1]);
      }
      return Math.min(cache[0], cache[1])
   };
}
...
const minCostClimbingStairs = new Solution().minCostClimbingStairs;
