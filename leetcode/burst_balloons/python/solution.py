...
class Solution:
    def maxCoins(self, numbers: list[int]) -> int:
        ...
        """
        memo = {}  # {(left, right): max coins for current range}
        ...
        def dfs(left, right):
            ...
            for index in range(left, right + 1):
                # number[index] is popped last not first
                coins = (
                    (numbers[left - 1] if left - 1 >= 0 else 1)
                    * (numbers[index])
                    * (numbers[right + 1] if right + 1 < len(numbers) else 1)
                )
                total = max(
                    total,
                    dfs(left, index - 1)
...
class Solution:
    def maxCoins(self, numbers: list[int]) -> int:
        ...
        """
        # {[left][right]: max coins for current range}
        cache = [[-1] * len(numbers) for _ in range(len(numbers))]
