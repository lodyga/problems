class Solution:
    def findNumberOfLIS(self, nums: list[int]) -> int:
        ...
        for left in range(N - 1, -1, -1):
            lis_length = cache[left][0]
            lis_counter = cache[left][1]

            for right in range(left + 1, N):
                cache_length = cache[right][0]
                cache_counter = cache[right][1]

                # check if LIS
                if nums[left] < nums[right]:
                    # longer LIS found
                    if lis_length < 1 + cache_length:
                        lis_length = 1 + cache_length
                        lis_counter = cache_counter
                        cache[left] = [lis_length, lis_counter]
                    # same length LIS found
                    elif lis_length == 1 + cache_length:
                        lis_counter += cache_counter
                        cache[left][1] = lis_counter

        max_len = max(length for length, _ in cache)
        return sum(count for length, count in cache if length == max_len)
    ...
    def findNumberOfLIS(self, nums: list[int]) -> int:
        ...
        for left in range(N - 1, -1, -1):
            for right in range(left + 1, N):
                ...
                if nums[left] < nums[right]:
                    # longer LIS found
                    if cache[left][0] < cache[right][0] + 1:
                        cache[left][0] = cache[right][0] + 1
                        cache[left][1] = cache[right][1]
                    # same length LIS found
                    elif cache[left][0] == cache[right][0] + 1:
                        cache[left][1] += cache[right][1]
...
class Solution:
    def findNumberOfLIS(self, nums: list[int]) -> int:
        ...
        def backtrack(start: int) -> None:
            nonlocal counter, subseq_len
            if len(subseq) > subseq_len:
                subseq_len = len(subseq)
                counter = 1
            elif len(subseq) == subseq_len:
                counter += 1
