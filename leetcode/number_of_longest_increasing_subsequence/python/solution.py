...
class Solution:
    def findNumberOfLIS(self, nums: list[int]) -> int:
        ...
        for left in range(len(nums) - 2, -1, -1):
            lis_len = cache[left][0]
            lis_counter = cache[left][1]
            
            for right in range(left + 1, len(nums)):
                if nums[left] >= nums[right]:
                    continue
                cache_len = cache[right][0]
                cache_counter = cache[right][1]
                
                # longer LIS found
                if lis_len < cache_len + 1:
                    lis_len = cache_len + 1
                    lis_counter = cache_counter
                    cache[left] = [lis_len, lis_counter]
                # same length LIS found
                elif lis_len == cache_len + 1:
                    lis_counter += cache_counter
                    cache[left][1] = lis_counter
            
        longest_lis_len = 1
        max_lis_counter = 0
        for lis_len, lis_counter in cache:
            if lis_len > longest_lis_len:
                longest_lis_len = lis_len
                max_lis_counter = lis_counter
            elif lis_len == longest_lis_len:
                max_lis_counter += lis_counter
