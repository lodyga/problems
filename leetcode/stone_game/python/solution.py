...
class Solution:
    def stoneGame(self, piles: list[int]) -> bool:
        ...
        def dfs(left, right, alice_to_move, alice_surplus):
            if left > right:
                return alice_surplus > 0

            if dfs(left + 1, right, not alice_to_move, alice_surplus + piles[left] * (1 if alice_to_move else -1)):
                return True
            if dfs(left, right - 1, not alice_to_move, alice_surplus + piles[left] * (1 if alice_to_move else -1)):
                return True
...
class Solution:
    def stoneGame(self, piles: list[int]) -> bool:
        ...
        def dfs(left, right, alice_to_move, alice_surplus):
            ...
            if dfs(left + 1, right, not alice_to_move, alice_surplus + (piles[left] if alice_to_move else -piles[left])):
                memo[(left, right)] = True
            elif dfs(left, right - 1, not alice_to_move, alice_surplus + (piles[right] if alice_to_move else -piles[right])):
                memo[(left, right)] = True
            return memo[(left, right)]
